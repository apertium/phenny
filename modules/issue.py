...
def issue(phenny, input):
    """ .issue <repository> <title> - create a new GitHub issue """    
    ...
    try:
        if not input.group(1):
            return phenny.reply('For which repository do I make this issue?')
        ...
        if len(ghpath) != 2:
            return phenny.reply('Sorry, that doesn\'t look like a valid repository path.')

        owner = ghpath[0]
        repo = ghpath[1]

        if owner not in allowed_owners:
            return phenny.reply('Sorry, you don\'t have permission to create an issue there.')

        title = " ".join(content.split()[1:]).strip()
        if len(title) < 1:
            return phenny.reply('What is the title of the issue?')
